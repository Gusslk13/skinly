...
import { Eye, EyeOff, ShieldAlert, Check, Loader2 } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  // Show "taking longer than expected" message after 15 s of waiting
  useEffect(() => {
    if (!googleLoading) { setGoogleSlow(false); return; }
    const t = setTimeout(() => setGoogleSlow(true), 15_000);
  ...
  // Google Sign-In is handled by the useGoogleAuth hook.
  // On Android: native account picker via Capacitor plugin.
  // On Web: Supabase OAuth redirect.
  ...
  return (
    <div className="min-h-screen bg-brand-gray-soft flex items-center justify-center p-4 sm:p-6 md:p-8 select-none relative overflow-hidden">
      ...
      <div className="w-full max-w-[440px] bg-brand-white p-8 sm:p-10 rounded-luxury shadow-xl border border-brand-black/5 relative z-10">
        ...
        {/* BOTÓN DE ACCESO SOCIAL */}
        <div className="mb-6">
          <button
            type="button"
            onClick={signInWithGoogle}
            disabled={loading || googleLoading}
            className={`w-full flex items-center justify-center gap-2.5 py-3 px-4 border rounded-luxury text-xs font-bold transition-all shadow-sm
              ${googleLoading
                ? 'bg-brand-gray-soft border-brand-black/10 text-brand-black/50 cursor-not-allowed opacity-80'
                : 'bg-brand-white border-brand-black/10 hover:border-brand-black hover:bg-brand-gray-soft text-brand-black cursor-pointer active:scale-98'
              }`}
          >
            {googleLoading ? (
              <Loader2 size={16} className="animate-spin text-brand-green-dark shrink-0" />
            ) : (
          ...
          {googleSlow && (
            <p className="mt-2 text-center text-[11px] text-brand-black/50 animate-pulse">
